...
// Auto-accept EULA and enable query protocol before every launch
function prepareServerFiles() {
    fs.writeFileSync('eula.txt', 'eula=true\n');
    const propsPath = 'server.properties';
    let props = fs.existsSync(propsPath) ? fs.readFileSync(propsPath, 'utf8') : '';
    const set = (key, val) => {
        if (new RegExp('^' + key + '=', 'm').test(props))
            props = props.replace(new RegExp('^' + key + '=.*', 'm'), key + '=' + val);
        else props += '\n' + key + '=' + val;
    };
    set('enable-query', 'true');
    set('query.port',   '25565');
    set('server-port',  '25565');
    fs.writeFileSync(propsPath, props);
    broadcast('[Absora Engine] EULA accepted. Query enabled.\n');
}
